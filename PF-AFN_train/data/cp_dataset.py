    ...
    def __getitem__(self, index):
        ...
        for key in ["paired"]:
            ...
            c[key] = transforms.Resize(
                self.fine_width, interpolation=InterpolationMode.BICUBIC
            )(c[key])
            cm[key] = Image.open(osp.join(self.data_path, "cloth-mask", c_name[key]))
            cm[key] = transforms.Resize(
                self.fine_width, interpolation=InterpolationMode.NEAREST
            )(cm[key])
        ...
        warped_cloth = transforms.Resize(
            self.fine_width, interpolation=InterpolationMode.BICUBIC
        )(warped_cloth)
        ...
        warped_cloth_mask = transforms.Resize(
            self.fine_width, interpolation=transforms.InterpolationMode.NEAREST
        )(warped_cloth_mask)
...
class CPDatasetTest(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # load image-parse-agnostic
        image_parse_agnostic = Image.open(
            osp.join(self.data_path, "image-parse-agnostic-v3.2", parse_name)
        )
        image_parse_agnostic = transforms.Resize(
            self.fine_width, interpolation=InterpolationMode.NEAREST
        )(image_parse_agnostic)
        ...
        densepose_map = Image.open(
            osp.join(self.data_path, "image-densepose", densepose_name)
        )
        densepose_map = transforms.Resize(
            self.fine_width, interpolation=InterpolationMode.BICUBIC
        )(densepose_map)
